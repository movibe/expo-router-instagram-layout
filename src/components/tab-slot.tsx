      ...
      {routes.map((route, index) => {
        ...
        return (
          <Screen
            ...
            style={[StyleSheet.absoluteFill, {overflow: 'hidden', zIndex: isFocused ? 0 : -1}, style]}
          >
            {descriptor.render()}
          </Screen>
        )
...
function useContextRoute(name: string) {
  ...
  if (!current) {
    console.warn(`Could not find route with name: ${name}. Options: ${state.routes.map(r => r.name).join(', ')}`)
  }
...
export function TabLink({
  ...
  return <Link {...props} href={buildLink(name)} onLongPress={onLongPress} onPress={onPress} />
}
